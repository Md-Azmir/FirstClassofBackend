...
//     res.send({status:"Books",result:books})
// })
// router.get('/:books/search', (req, res) => {
//     const name = req.query.name.toLowerCase();
//     const result = books.filter(book => book.name.toLowerCase().includes(name));
//     res.send({result});
// });
// router.get('/:movies',(req,res)=>{
//     res.send({status:"Movies",result:movies})
// })
// router.get('/:movies/search', (req, res) => {
//   const name = req.query.name.toLowerCase();
//   const result = movies.filter(movie => movie.name.toLowerCase().includes(name));
//   res.send({result});
// });
router.get('/books', function(req, res) {
  ...
  for(let i=0;i<books.length;i++){
    if(books[i].name==name){
      return res.send({status:"Books",message:books[i]})
     }
  }
   return res.send({status:'Movies',message:books});
});
